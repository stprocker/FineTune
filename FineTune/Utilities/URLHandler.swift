...
final class URLHandler {
    ...
    private func handleSetVolumes(queryItems: [URLQueryItem]) {
        ...
        for (identifier, volumePercent) in pairs {
            // Linear conversion: volume=100 -> gain 1.0, volume=200 -> gain 2.0
    ...
    /// Adjusts by 5% slider position using the dB-based VolumeMapping curve
    ...
    private func handleSetMute(queryItems: [URLQueryItem]) {
        ...
        for (identifier, muted) in pairs {
            if let app = findApp(by: identifier) {
                ...
                // App not active - persist via settingsManager for when it launches
                audioEngine.settingsManager.setMute(for: identifier, to: muted)
    ...
    private func handleToggleMute(queryItems: [URLQueryItem]) {
        ...
        for identifier in identifiers {
            if let app = findApp(by: identifier) {
                ...
                // App not active - toggle persisted mute state
                let current = audioEngine.settingsManager.getMute(for: identifier) ?? false
                audioEngine.settingsManager.setMute(for: identifier, to: !current)
        ...
        if identifiers.isEmpty {
            ...
            for identifier in identifiers {
                if let app = findApp(by: identifier) {
                    ...
                    // App not active - persist reset via settingsManager
                    audioEngine.settingsManager.setVolume(for: identifier, to: 1.0)
                    audioEngine.settingsManager.setMute(for: identifier, to: false)
