...
final class AudioEngineRoutingTests: XCTestCase {
    ...
    func testSetDeviceRevertsCleansPersistedRouting() {
        ...
        // The revert path for "no previous routing" removes the key from
        // appDeviceRouting, but the settingsManager still has the persisted value
        // because the revert code doesn't clear it for the nil-previous case.
        // This is acceptable: applyPersistedSettings will attempt the device again
        // on next launch and clean up if it still fails.
        //
        // For the "has previous routing" case (Bug 3 with prior device),
        // settingsManager gets reverted to the previous device UID.
    }
}
